...
* **Desktop multi-window:** `desktop_multi_window`, `window_manager`, `tray_manager`, `local_notifier`, Linux/Wayland focus bugs, tray icon resolution, `sqflite_common_ffi` desktop databases.
* **Printing:** `pdf` + `printing` plugins, printer list selection, `Printing.directPrintPdf()` vs `layoutPdf()`.
* **Linux plugin bugs:** `desktop_webview_window` use-after-free crash and OpenGL `FlView` patch.
...
* For desktop multi-window on **stable**: always use `desktop_multi_window: ^0.3.0`. Native Windowing API (`RegularWindowController`, etc.) is `@internal`/experimental — only available on channel `main`.
* Validate third-party plugin platform support in `pubspec.yaml` before recommending.
...
| App crashes on WebView window close (Linux) | use-after-free + OpenGL FlView bug | Apply C++ patch from [desktop_plugin_bugs.md](./references/desktop_plugin_bugs.md) |
...
* [desktop_plugin_bugs.md](./references/desktop_plugin_bugs.md) — Linux plugin bug catalog: `desktop_webview_window` use-after-free segfault + OpenGL FlView crash — C++ patch instructions for `.pub-cache`.
