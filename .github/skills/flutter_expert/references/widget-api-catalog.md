...
*   **[desktop_plugin_bugs.md](references/desktop_plugin_bugs.md)** — Bugs conocidos de plugins Flutter Desktop en Linux con parches C++ obligatorios: `desktop_webview_window` use-after-free Segfault + colapso OpenGL por `FlView` embebido, tabla de versiones afectadas, instrucciones de localización del archivo en `.pub-cache`, código de parche listo para copiar, y protocolo de contingencia si el parche se pierde tras `pub cache clean`.
